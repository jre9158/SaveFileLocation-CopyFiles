...
type
  TINCTimeResp6RepForm = class(TAlpineBaseSummaryReportForm)
    TitleBand: TQRBand;
...
var
  INCTimeResp6RepForm: TINCTimeResp6RepForm;
...
procedure TINCTimeResp6RepForm.FormCreate(Sender: TObject);
var Form            : TForm;
    TimeDiff        : Real;
    ...
    Try
      If (GetField('NFIRSMAIN',EndField).AsDateTime > 0) and (GetField('NFIRSMAIN',StartField).AsDateTime > 0) then begin
        If CheckInTime(GetField('NFIRSMAIN',StartField).AsDateTime) then begin
          ...
          ArrayNum        := Trunc(TimeDiffSeconds/6) + 1;
...
function TINCTimeResp6RepForm.CheckInTime(TimeVar: TDateTime): Boolean;
...
procedure TINCTimeResp6RepForm.TimeBandNeedData(Sender: TObject; var MoreData: Boolean);
Var StartTime   : TDateTime;
    EndTime     : TDateTime;
    StartString : String;
begin
  inherited;
  StartTime               := (ArrayRow - 1) / (10 * 60 * 24);
  EndTime                 := (ArrayRow - 0) / (10 * 60 * 24);
  ...
  If TotalVar > 0 then begin
    ...
    SubAccVar               := SubAccVar + TimeArray[ArrayRow] 
  end;
  CumPercentField.Caption := FormatFloat('##0.00',TotalPercent);
  SubAccField.Caption     := FormatFloat('#,##0',SubAccVar);
...
procedure TINCTimeResp6RepForm.SummaryBandBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  inherited;
...
procedure TINCTimeResp6RepForm.BaseReportBeforePrint(Sender: TCustomQuickRep; var PrintReport: Boolean);
begin
  inherited;
  ArrayRow     := 1;
  TotalPercent := 0;
...
procedure TINCTimeResp6RepForm.GetStartField;
var Form     : TForm;
begin
  Form                  := GetFormOfType(TIncCondForm);
...
procedure TINCTimeResp6RepForm.GetEndField;
var Form     : TForm;
begin
  Form                  := GetFormOfType(TIncCondForm);
